...
class Result(Page):
    ...
    def on_init(self: any) -> None:
        ...
        self.win_img = pygame.image.load(os.path.join(
            ROOT_DIR, 'assets/wrecked_robot.png')).convert_alpha()
        self.lose_img = pygame.image.load(os.path.join(
            ROOT_DIR, 'assets/logo.png')).convert_alpha()
    ...
    def on_start(self: any) -> None:
        if self.game.client is None or self.game.client.info is None or 'lobby' not in self.game.client.info:
            if self.game.log is not None:
                self.game.log.error(
                    f'Invalid state for result page, client lobby info missing')
        ...
        self.text = 'KI gewinnt!' if self.lobby['winner'] == 'ai' else 'Menschen gewinnen!' if self.lobby[
            'winner'] == 'humans' else 'Niemand gewinnt!'
    ...
    def draw(self: any) -> None:
        ...
        img_pos = (popup_pos[0] + popup_dim[0] / 2 -
                   img_size / 2, popup_pos[1] + title_padding)
        self.game.draw(pygame.transform.smoothscale(
            self.img, (img_size, self.img.get_height() / self.img.get_width() * img_size)), img_pos)
        text = self.game.text_surface(
            self.text, 'Arial', title_fs / 2, Colors.purple)
        self.game.draw(text, (popup_pos[0] + popup_dim[0] / 2 -
                       text.get_width() / 2, img_pos[1] + img_size + title_padding))
        ...
        again_btn_pos = (popup_pos[0] + popup_dim[0] / 2 - self.again_btn_dim[0] / 2,
                         img_pos[1] + img_size + text.get_height() + 2 * title_padding)
        self.again_btn = self.game.create_btn(again_btn_pos, self.again_btn_dim, Colors.purple, round(
            10 * min_ratio), 'Erneut spielen', 'Arial', round(20 * min_ratio), Colors.salmon)
        self.back_btn = self.game.create_btn((popup_pos[0] + popup_dim[0] / 2 - self.back_btn_dim[0] / 2, again_btn_pos[1] + self.again_btn_dim[1] +
                                             title_padding), self.back_btn_dim, Colors.salmon, round(10 * min_ratio), 'Zurück', 'Arial', round(20 * min_ratio), Colors.white)

    def iteration(self: any) -> None:
        if self.again_btn.collidepoint(pygame.mouse.get_pos()):
            if self.again_btn_hover.switch_true():
                self.again_btn_dim = (
                    self.base_btn_dim[0] * 1.1, self.base_btn_dim[1] * 1.1)
