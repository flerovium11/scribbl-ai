...
class Guess(Page):
    ...
    def on_init(self:any)->None:
        ...
        self.text_input = Input(self.game, self, placeholder='Errate das Wort...', disabled_color=Colors.salmon)
    ...
    def draw(self:any)->None:
        self.check_network_connection([LobbyState.CHOOSE_WORD.name, LobbyState.GAME.name], [PlayerRole.GUESSER.name])
        ...
        playerlist_dim = (self.game.dim[0] / 3, self.game.dim[1] - title_padding * 6 - title.get_height())
        ...
        canvas_dim = (self.game.dim[0] - playerlist_dim[0] - 3 * title_padding, playerlist_dim[1])
        grid = self.clt.lobby['grid'] if self.clt.lobby['grid'] is not None else {'tiles': [[0]], 'dim': (1, 1)}
        ...
        input_dim = (self.game.dim[0] / 2, self.game.dim[1] - playerlist_pos[1] - playerlist_dim[1] - 2 * title_padding)
        ...
        if self.clt.game_state == LobbyState.GAME.name:
            if self.text_input.manager.value != '':
                pygame.draw.circle(self.game.screen, Colors.red if self.clt.has_guessed else Colors.purple, (self.done_btn.center), self.done_btn_rad)
                self.game.draw(done_btn_text, (self.done_btn.center[0] - done_btn_text.get_width() // 2, self.done_btn.center[1] - done_btn_text.get_height() // 2))
            
            self.text_input.font_size = input_dim[1] - title_padding
            self.text_input.disabled = self.clt.has_guessed
            self.text_input.draw(input_pos, input_dim[0], (title_padding / 2, title_padding / 2), 10)
