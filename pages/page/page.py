...
class Page:
    ...
    def draw(self: any) -> None:
        ...
        surface = self.game.text_surface(
            f'Page {self.name}', 'Ink Free', 100, Colors.purple)
        self.game.screen.blit(
            surface, (self.game.dim[0]/2 - surface.get_width()/2, self.game.dim[1]/2 - surface.get_height()/2))
        self.btn = self.game.create_btn(
            (20, 20), self.btn_dim, Colors.salmon, 10, 'Zurück', 'Arial', 30, Colors.purple)
    ...
    def check_network_connection(self: any, allow_states: list[LobbyState] = [], allow_roles: list[PlayerRole] = []) -> None:
        ...
        right_role = self.clt.info['lobby']['players'][self.clt.info['id']
                                                       ]['role'] in allow_roles if right_state else False
        ...
        if connected and in_lobby:
            ...
            if not right_role or not right_state:
                if self.game.log is not None:
                    self.game.log.error(
                        f'Invalid {"state " + self.clt.game_state if not right_state else "player role " + self.clt.player["role"]} for page {self.game.pagename} - allowed: {str(allow_states) if not right_state else str(allow_roles)}')
