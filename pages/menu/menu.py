...
class Menu(Page):
    ...
    def draw(self: any) -> None:
        ...
        title_fs = round(
            min(180, self.game.dim[0] / 5.147, self.game.dim[1] / 4.583))
        self.modebtn_dim = tuple(
            [val*title_fs/120 for val in self.base_modebtn_dim])
        ...
        subtitle = self.game.text_surface(
            'Bist du besser als die KI?', 'Ink Free', title_fs / 4, Colors.purple)
        ...
        self.game.draw(pygame.transform.rotate(title_bg, self.rotate_bg /
                       120 * title_fs), (title_x, title_padding_top + title_fs/12))
        self.game.draw(pygame.transform.rotate(
            title, self.rotate_title / 120 * title_fs), (title_x, title_padding_top))

        modebtn_y = self.game.dim[1]/2 - self.modebtn_dim[1]/2
        modebtn1_x = self.game.dim[0]/2 - self.modebtn_dim[0] - \
            self.buttons_distance/2 + \
            self.modebtn_dim[0]/2 - self.modebtn1_dim[0]/2
        modebtn1_y = modebtn_y + self.modebtn_dim[1]/2 - self.modebtn1_dim[1]/2
        self.modebtn1 = self.game.create_btn((modebtn1_x, modebtn1_y), self.modebtn1_dim,
                                             Colors.purple, self.modebtn_radius, 'Mehrspieler', 'Arial', title_fs / 4, Colors.salmon)

        if self.multiplayer_info is not None:
            multiplayer_info_text = self.game.text_surface(
                self.multiplayer_info, 'Arial', title_fs / 8, Colors.salmon)
            info_dim = (multiplayer_info_text.get_width(
            ) + title_padding_top, multiplayer_info_text.get_height() * 1.5)
            info_pos = (title_fs / 10,
                        self.game.dim[1] - info_dim[1] - title_fs / 10)
            self.game.create_btn(info_pos, info_dim, Colors.black, 10,
                                 self.multiplayer_info, 'Arial', title_fs / 8, Colors.white)

        modebtn2_x = self.game.dim[0]/2 + self.buttons_distance / \
            2 + self.modebtn_dim[0]/2 - self.modebtn2_dim[0]/2
        modebtn2_y = modebtn_y + self.modebtn_dim[1]/2 - self.modebtn2_dim[1]/2
        self.modebtn2 = self.game.create_btn((modebtn2_x, modebtn2_y), self.modebtn2_dim,
                                             Colors.salmon, self.modebtn_radius, 'Sandkiste', 'Arial', title_fs / 4, Colors.purple)

        self.qbtn = pygame.Rect(self.game.dim[0] // 2 - self.qbr, self.game.dim[1] - (
            140 * title_fs / 120) - self.qbr, 2 * self.qbr, 2 * self.qbr)
        pygame.draw.circle(self.game.screen, Colors.pink,
                           (self.qbtn.center), self.qbr)
        q_surface = self.game.text_surface(
            '?', 'Arial', title_fs / 2, Colors.beige)
        self.game.draw(q_surface, (self.qbtn.center[0] - q_surface.get_width(
        ) // 2, self.qbtn.center[1] - q_surface.get_height() // 2))

        credits = self.game.text_surface(
            'Ennio Binder 2024', 'Ink Free', title_fs / 6, Colors.purple)
        credits_x = self.game.dim[0] // 2 - credits.get_width() // 2
        self.game.draw(
            credits, (credits_x, self.game.dim[1] - credits.get_height() - 10))

    def iteration(self: any) -> None:
        if self.modebtn1.collidepoint(self.mouse_pos):
            if self.modebtn1_hover.switch_true():
                self.modebtn1_dim = tuple(
                    [val * 1.05 for val in self.modebtn_dim])
