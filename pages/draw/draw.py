...
class Draw(Page):
    ...
    def draw(self: any) -> None:
        self.check_network_connection(
            [LobbyState.CHOOSE_WORD.name, LobbyState.GAME.name], [PlayerRole.DRAWER.name])
        ...
        playerlist_dim = (
            self.game.dim[0] - canvas_size - 3 * title_padding, self.game.dim[1] - title_padding * 3 - title.get_height())
        ...
        self.clt.grid = {'tiles': compress_grid(self.canvas.grid), 'dim': (
            self.canvas.grid_width, self.canvas.grid_height), 'compressed': True}
        ...
        if self.choosing:
            ...
            pygame.draw.rect(self.game.screen, Colors.beige,
                             popup_rect, border_radius=popup_bdrad)
            text = self.game.text_surface(
                'Wähle ein Wort, das du zeichnen willst!', 'Ink Free', title_fs / 2, Colors.purple)
            self.game.draw(
                text, (popup_pos[0] + title_padding, popup_pos[1] + title_padding))
            words = self.clt.info['lobby']['words']
            word_count = len(words)
            self.base_word_btn_dim = (
                (popup_dim[0] - title_padding * 2) / word_count - title_padding * 0.5, popup_dim[1] / 2 - title_padding * 2)
            ...
            self.word_btns = []
            btn_y = (popup_pos[1] + title_padding +
                     text.get_height() + popup_dim[1]) / 2

            for i, word in enumerate(words):
                button = self.game.create_btn((popup_pos[0] + title_padding + i * (self.base_word_btn_dim[0] + title_padding * 0.5), btn_y), self.word_btn_dims[i], Colors.purple, round(
                    10 * min_ratio), word, 'Arial', round(20 * min_ratio), Colors.salmon, auto_fontsize=True, padding_x=int(10 * min_ratio))
                self.word_btns.append(button)

            Timer.draw(self.game, (popup_pos[0] + popup_dim[0] - title_padding * 3, popup_pos[1] + title_padding),
                       title_padding, Colors.pink, Colors.black, self.clt.lobby['countdown'], self.clt.lobby['choose_word_time'])
        else:
            self.word_btns = None
            info = f'Schnell, zeichne "{self.clt.lobby["word"]}"!' if self.clt.lobby[
                'word'] is not None else "Schnell, zeichneeeeee..."
            ...
            for i, btn in enumerate(self.word_btns):
                if btn.collidepoint(self.mouse_pos):
                    if self.word_btns_hover[i].switch_true():
                        self.word_btn_dims[i] = tuple(
                            [val * 1.05 for val in self.base_word_btn_dim])
