    ...
    def draw(self:any)->None:
        ...
        if 'mode' in self.game.client.info and self.game.client.info['mode'] == 'lobby':
            ...
            if self.game.client.info['lobby']['state'] == LobbyState.WAITING.name:
                ...
                info_text = self.game.text_surface(bottom_info, 'Arial', title_fs / 2.5, Colors.purple)
                ...
                playerlist_pos = (title_padding, title_padding * 2 + self.exit_btn_dim[1])
                playerlist_dim = (self.game.dim[0] - 2 * title_padding, self.game.dim[1] - playerlist_pos[1] - title_padding * 4)
                self.game.playerlist.draw(playerlist_pos, playerlist_dim, 10, players, self.game.client.info['id'], 5, title_padding * 2, (title_padding, title_padding), 3, Colors.white, Colors.salmon, Colors.gray, Colors.black)

                status_info_text = self.game.text_surface(status, 'Arial', title_fs / 3, Colors.salmon)
        ...
        if self.game.client is not None and self.game.client.name is None:
            ...
            text = self.game.text_surface('Wie möchtest du genannt werden?', 'Ink Free', title_fs / 2, Colors.purple)
            ...
            self.name_input.draw((popup_pos[0] + title_padding, input_y), popup_dim[0] - 4 * title_padding, (title_padding, title_padding / 2), 10)
            self.base_name_btn_dim = (popup_dim[0] / 3 - 1.5 * title_padding, popup_dim[1] - input_y + popup_pos[1] - self.name_input.rect.height - 4 * title_padding)
            ...
            cancel_btn_pos = (popup_pos[0] + popup_dim[0] - (1 + show_enter) * title_padding - 2 * self.base_name_btn_dim[0] + (self.base_name_btn_dim[0] if not show_enter else 0), popup_pos[1] + popup_dim[1] - self.base_name_btn_dim[1] - title_padding)
            self.cancel_btn = self.game.create_btn(cancel_btn_pos, self.cancel_btn_dim, Colors.red, round(10 * min_ratio), 'Abbrechen', 'Arial', round(20 * min_ratio), Colors.black)

            if show_enter:
                self.enter_btn = self.game.create_btn((cancel_btn_pos[0] + self.base_name_btn_dim[0] + title_padding, cancel_btn_pos[1]), self.enter_btn_dim, Colors.purple, round(10 * min_ratio), 'Bestätigen', 'Arial', round(20 * min_ratio), Colors.salmon)
        else:
            self.choosing_name = False
            self.cancel_btn = None
            self.enter_btn = None
    ...
    def iteration(self:any)->None:
        ...
        if self.cancel_btn is not None and self.cancel_btn.collidepoint(self.mouse_pos):
            if self.cancel_btn_hover.switch_true():
                self.cancel_btn_dim = tuple([val * 1.05 for val in self.base_name_btn_dim])
                pygame.mouse.set_cursor(pygame.SYSTEM_CURSOR_HAND)
        ...
        if self.enter_btn is not None and self.enter_btn.collidepoint(self.mouse_pos):
            if self.enter_btn_hover.switch_true():
                self.enter_btn_dim = tuple([val * 1.05 for val in self.base_name_btn_dim])
                pygame.mouse.set_cursor(pygame.SYSTEM_CURSOR_HAND)
