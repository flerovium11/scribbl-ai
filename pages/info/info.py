...
class Info(Page):
    ...
    def draw(self: any) -> None:
        ...
        self.btn = self.game.create_btn((title_padding - (self.btn_dim[0] - self.base_btn_dim[0])*0.5, title_padding - (
            self.btn_dim[1] - self.base_btn_dim[1])*0.5), self.btn_dim, Colors.purple, round(10 * min_ratio), 'Zurück', 'Arial', round(30 * min_ratio), Colors.salmon)
        ...
        element_flow = []
        element_flow.append({'py': h1_py, 'el': self.game.text_surface(
            'Zweck', 'Ink Free', h1_fs, Colors.purple)})
        self.paragraph('ScribblAI ist ein Spiel, das die Anwendungsmöglichkeiten von Bilderkennung mit KI und Multiplayer-Netzwerktechnik erforscht. Es wurde 2024 im Rahmen der Coding_Academy am WIFI Gmunden erschaffen.',
                       'Arial', p_fs, Colors.black, p_py, self.game.dim[0] / (p_fs * 0.5), element_flow)
        element_flow.append({'py': h1_py, 'el': self.game.text_surface(
            'Spielkonzept', 'Ink Free', h1_fs, Colors.purple)})
        self.paragraph('Eine Person muss ein Wort zeichnen und die anderen müssen es erraten. Das Bild wird zusätzlich durch ein CNN geschickt, und sollte die KI das Bild richtig klassifizieren, verlieren alle menschlichen Spieler*innen. Das Ziel ist es folglich, so zu zeichnen, dass Menschen das Wort erraten können, aber kein Computeralgorithmus.',
                       'Arial', p_fs, Colors.black, p_py, self.game.dim[0] / (p_fs * 0.5), element_flow)
        element_flow.append({'py': h1_py, 'el': self.game.text_surface(
            'Danksagungen', 'Ink Free', h1_fs, Colors.purple)})
...
Grundlegende Spielidee: skribbl.io""", 'Arial', p_fs, Colors.black, p_py, self.game.dim[0] / (p_fs * 0.5), element_flow)
        ...
        posx = (self.game.dim[0] - max([el['el'].get_width()
                for el in element_flow])) / 2
    ...
    def paragraph(self: any, text: str, font: str, fs: int, color: Colors, pad: int, maxlen: int, element_flow: list) -> None:
        ...
        for word in words:
            if len(line + word + ' ') > maxlen or (len(word) and '\n' in word):
                element_flow.append(
                    {'py': pad, 'el': self.game.text_surface(line[:-1], font, fs, color)})
                line = word.replace('\n', '') + ' '
            else:
                line += word + ' '
