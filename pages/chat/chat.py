...
class Message:
    ...
    def draw(self: any, pos: list[int], title_padding: float) -> None:
        sender = self.game.text_surface(
            self.sender, 'Arial', 15, self.sender_color)
        text = self.game.text_surface(self.text, 'Arial', 20, self.text_color)

        if self.is_image:
            dim = (self.base_side, self.base_side + sender.get_height() +
                   3 * self.padding[1] + text.get_height())
            ...
            pygame.draw.rect(self.game.screen, self.color,
                             self.message, border_radius=bdrad)
            self.canvas.draw((pos[0] + self.padding[0], pos[1] + sender.get_height() + 2 * self.padding[1]), (dim[0] -
                             2 * self.padding[0], dim[1] - 4 * self.padding[1] - sender.get_height() - text.get_height()), Colors.white)
            self.game.draw(text, (pos[0] + self.padding[0], pos[1] +
                           dim[1] - self.padding[1] - text.get_height()))
        else:
            min_width = self.game.dim[0] / 6
            dim = (max(min_width, max(text.get_width(), sender.get_width()) + 2 *
                   self.padding[0]), text.get_height() + sender.get_height() + 3 * self.padding[1])
            self.message = pygame.Rect(pos[0], pos[1], dim[0], dim[1])
            bdrad = 10
            self.message.inflate(-2 * bdrad, -2 * bdrad)
            pygame.draw.rect(self.game.screen, self.color,
                             self.message, border_radius=bdrad)
            self.game.draw(
                text, (pos[0] + self.padding[0], pos[1] + sender.get_height() + 2 * self.padding[1]))

        self.game.draw(
            sender, (pos[0] + self.padding[0], pos[1] + self.padding[1]))
        pygame.draw.polygon(self.game.screen, self.color, [(self.message.x + title_padding * 0.5, self.message.y), (
            self.message.x, self.message.y + title_padding * 0.5), (self.message.x - title_padding * 0.5, self.message.y - title_padding * 0.5)])
...
class Chat(Page):
    ...
    def on_start(self: any) -> None:
        if self.game.client is None or self.game.client.info is None or 'lobby' not in self.game.client.info:
            if self.game.log is not None:
                self.game.log.error(
                    f'Invalid state for chat page, client lobby info missing')

            self.game.return_info = 'Ungültiger Zustand'
            self.game.goto_page('Menu')

        self.lobby = self.game.client.info['lobby']
        grid = self.lobby['grid'] if self.lobby['grid'] is not None else {
            'tiles': [[0]], 'dim': (1, 1)}
        ...
        for player in sorted(self.lobby['players'], key=lambda x: x['role']):
            if player['role'] == PlayerRole.DRAWER.name:
                self.messages.append(Message(
                    self.game, self.lobby['word'], player['name'], (10, 10), is_image=True, canvas=self.canvas))
            else:
                self.messages.append(
                    Message(self.game, player['guess'], player['name'], (10, 5)))

        self.messages.append(Message(
            self.game, self.lobby['ai_guess'] + f' ({round(self.lobby["ai_certainty"] * 100)}%)', 'KI', (10, 5), color=Colors.pink))
    ...
    def animation(self: any) -> None:
        ...
        for message in self.messages:
            self.animate(message.pos, (0, y), 40, 50,
                         self.regression, self.logistic, 1)
    ...
    def animate(self: any, pos: list[float], end: tuple[float], x_steps: int, y_steps: int, x_func: callable, y_func: callable, time: float) -> None:
        ...
        for i in range(steps):
            x = start[0] + (end[0] - start[0]) * \
                x_func(min(i, x_steps) / x_steps)
            y_factor = y_func(min(i, y_steps) / y_steps)
            y = start[1] + (end[1] - start[1]) * y_factor
            self.chat_scroll = start_scroll + \
                (goal_scroll - start_scroll) * y_factor
            pos[0] = x
            pos[1] = y
            self.trigger_update()
            sleep(time / steps)
    ...
    def draw(self: any) -> None:
        ...
        for i, message in enumerate(self.messages):
            message.draw(
                (title_padding + message.pos[0], title_padding + message.pos[1] + self.chat_scroll), title_padding)
        ...
        if self.animation_done:
            ...
            self.continue_btn = self.game.create_btn((self.game.dim[0] - title_padding - self.continue_btn_dim[0], self.game.dim[1] - title_padding -
                                                     self.continue_btn_dim[1]), self.continue_btn_dim, Colors.purple, int(20 * min_ratio), 'Weiter >', 'Arial', 30 * min_ratio, Colors.beige)
    ...
    def iteration(self: any) -> None:
        if self.continue_btn is not None and self.continue_btn.collidepoint(self.mouse_pos):
            if self.continue_btn_hover.switch_true():
                self.continue_btn_dim = tuple(
                    [val * 1.05 for val in self.base_continue_btn_dim])
        ...
        if self.animation_done:
            if event.type == pygame.MOUSEWHEEL:
                self.chat_scroll = max(
                    min(0, self.chat_scroll + event.y * 10), -self.messages[-1].pos[1])
                self.trigger_update()
