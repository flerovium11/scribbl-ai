...
class Lobby:
    ...
    def remove_player(self: any, player: any) -> None:
        if self.state is LobbyState.WAITING and len(
                self.players
        ) > player.id:  # players are only removed in waiting state
    ...
    def main(self: any) -> None:
        ...
        while self.state is LobbyState.GAME:
            try:
                ...
                if self.countdown == 0 or all([
                        player.has_guessed or player.role is PlayerRole.DRAWER
                    ...
                    try:
                        ...
                        grid = decompress_grid(
                            grid['tiles'], grid_width
                        ) if 'compressed' in grid else grid['tiles']
                        prediction = self.model.predictImage(
                            image.format_for_ai(grid) if self.grid is not None
                            else np.array([[0]]))  # else an empty white
                        self.ai_guess = prediction[0]['category']
                        self.ai_certainty = float(prediction[0]['certainty'])
                        players_guessed = any([
                            player.guess == self.word
                            for player in self.players
                        ])
            ...
            if hasattr(self, 'id'):
                ...
                packet['lobby'] = {
                    ...
                    'players': [{
                        ...
                        player.online,
                        'role':
                        player.role.name if player.role is not None else None,
                        'guess':
                        player.guess,
                        'has_guessed':
                        player.has_guessed
...
class Server:
    ...
    def start(self: any) -> None:
        ...
        while self.running:
            try:
                print(*[
                    f'lobby {lobby.id} ({len(lobby.players)}) {lobby.state.name}'
        ...
        while self.running:
            try:
                ...
                sorted_lobbies = sorted(
                    self.lobbies,
                    key=cmp_to_key(lambda lobby1, lobby2: len(lobby1.players) -
                                   len(lobby2.players)))
                open_lobbies = list(
                    filter(
                        lambda lobby: lobby.state == LobbyState.WAITING and
