...
print('Loading keras, this can take a while... (especially on cold start)')  # nopep8
import keras.models
...
categories_file = open(os.path.join(
    EXTERNAL_DIR, 'categories_german.txt'), encoding='utf-8')
...
class AI:
    ...
    def shapeImage(self: any, img: np.array) -> np.array:
        # squash the image down to 28x28 pixels
        img = resize(img, (28, 28), anti_aliasing=True)
        # imgplot = plt.imshow(img, cmap='gray')
        # plt.show()
        # shape array for inputting into model
    ...
    def prediction(self: any, out: np.array) -> dict[str, any]:
        prediction = [{'certainty': number, 'category': self.categories[index]}
                      for index, number in enumerate(out)]
        prediction = sorted(
            prediction, key=lambda x: x['certainty'], reverse=True)
        return prediction
