...
class Input:
    ...
    def draw(self:any, pos:tuple[float], width:float, padding:tuple[float], bdrad:float)->None:
        ...
        self.text_surface = self.game.text_surface(self.manager.value[:self.manager.cursor_pos] + ('|' if not self.disabled and pygame.time.get_ticks() % 1000 < 500 else '') + self.manager.value[self.manager.cursor_pos:], self.font, self.font_size, self.font_color)
        ...
        pygame.draw.rect(self.game.screen, self.disabled_color if self.disabled else self.color, self.rect, border_radius=bdrad)

        if self.placeholder is not None and self.manager.value == '':
            self.placeholder_surface = self.game.text_surface(self.placeholder, self.font, self.font_size, self.placeholder_color)
            self.game.draw(self.placeholder_surface, (pos[0] + padding[0], pos[1] + padding[1]))
