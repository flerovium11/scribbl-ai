...
class Canvas:
    def __init__(self: any, game: any, page: any, readonly: bool = False, grid_width: int = 200, grid_height: int = 200) -> None:
        ...
        self.mode = 'draw'
        self.pencil_img = pygame.image.load(
            './assets/pencil.png').convert_alpha()
        self.eraser_img = pygame.image.load(
            './assets/eraser.png').convert_alpha()
        self.trash_img = pygame.image.load(
            './assets/trash.png').convert_alpha()
        self.drawing = self.ai_predict = False
        self.grid_width = grid_width
        self.grid_height = grid_height
        self.grid = self.empty_grid(self.grid_width, self.grid_height)
        self.draw_radius_proportion = 0.02
            ...
            for y in range(min_y, max_y + 1):
                ...
                if distance <= radius:
                    if erase:
                        ...
                        darkness = 1 - (distance / radius) * fade_factor
    ...
    def draw(self: any, pos: tuple[float], dim: tuple[float], color: Colors) -> None:
        self.canvas = pygame.Rect(
            pos[0], pos[1], dim[0], dim[1] * (1 if self.readonly else 0.875))
        bdrad = 10
        self.canvas.inflate(-2 * bdrad, -2 * bdrad)
        pygame.draw.rect(self.game.screen, color,
                         self.canvas, border_radius=bdrad)
        ...
        if not self.readonly:
            ...
            self.pencil_size = (icon_size * (1 + self.phover.state / 10),
                                icon_size * (1 + self.phover.state / 10))
            eraser_size = (icon_size * (1 + self.ehover.state / 10),
                           icon_size * (1 + self.ehover.state / 10))
            trash_size = (icon_size * (1 + self.thover.state / 10),
                          icon_size * (1 + self.thover.state / 10))
            ...
            self.game.draw(pygame.transform.smoothscale(
                self.pencil_img, self.pencil_size), pencil_pos)
            self.game.draw(pygame.transform.smoothscale(
                self.eraser_img, eraser_size), eraser_pos)
            self.game.draw(pygame.transform.smoothscale(
                self.trash_img, trash_size), trash_pos)
        ...
        if not self.readonly:
            if self.hover.state and hasattr(self.page, 'mouse_pos'):
                ...
                self.game.draw(pygame.transform.smoothscale(img, tuple(
                    [val / 1.5 for val in self.pencil_size])), (self.page.mouse_pos[0], self.page.mouse_pos[1] - self.pencil_size[1] / 2 - 5))

            if pygame.time.get_ticks() - self.scrolled_time <= 1000 and hasattr(self.page, 'mouse_pos'):
                pygame.draw.circle(self.game.screen, Colors.pink, self.page.mouse_pos, (self.draw_radius if self.mode ==
                                   'draw' else self.erase_radius) * self.canvas.width / self.grid_width + 2, width=1)

    def normalize_pos(self: any, pos: tuple[float]) -> tuple[float]:
        pos_x = math.floor((pos[0] - self.canvas.x) /
                           self.canvas.width * self.grid_width)
        pos_y = math.floor((pos[1] - self.canvas.y) /
                           self.canvas.height * self.grid_height)
        return (pos_x, pos_y)
    ...
    def event_check(self: any, event: pygame.event) -> None:
        ...
        if event.type == pygame.MOUSEWHEEL:
            if self.mode == 'draw':
                self.draw_radius = min(self.max_draw_radius, max(
                    self.min_draw_radius, self.draw_radius + event.y))

            if self.mode == 'erase':
                self.erase_radius = min(self.max_erase_radius, max(
                    self.min_erase_radius, self.erase_radius + event.y))
        ...
        if self.hover.state:
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if self.drawing:
                    ...
                    dist_x, dist_y = normalized[0] - \
                        self.last_pos[0], normalized[1] - self.last_pos[1]
                    ...
                    for d in range(1, cuts):
                        perc = d / cuts
                        pos = self.coords(
                            self.last_pos[0] + round(dist_x * perc), self.last_pos[1] + round(dist_y * perc))
                        self.draw_circle(pos, radius, self.mode != 'draw', 0.1)

                    self.draw_circle(
                        (normalized[0], normalized[1]), radius, self.mode != 'draw', 0.1)
    ...
    def draw_trace(self: any) -> None:
        for x in range(self.grid_width):
            for y in range(self.grid_height):
                if self.grid[x][y] > 0:
                    rect = pygame.Rect(self.canvas.x + x / self.grid_width * self.canvas.width, self.canvas.y + y / self.grid_height *
                                       self.canvas.height, math.ceil(self.canvas.width / self.grid_width), math.ceil(self.canvas.height / self.grid_height))
                    brightness = int(255 - self.grid[x][y] * 255)
                    pygame.draw.rect(self.game.screen, pygame.Color(
                        brightness, brightness, brightness), rect)

    def start_drawing(self: any) -> None:
        self.drawing = True
        normalized = self.normalize_pos(self.page.mouse_pos)
        radius = self.draw_radius if self.mode == 'draw' else self.erase_radius
        self.draw_circle((normalized[0], normalized[1]),
                         radius, self.mode != 'draw', 0.1)
        self.last_pos = normalized
