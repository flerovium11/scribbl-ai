...
class PlayerList:
    ...
    def draw(self: any,
        ...
        pygame.draw.rect(self.game.screen, bg_color,
                         self.rect, border_radius=bdrad)

        player_dim = (dim[0] - 2 * padding[0], min(player_max_height,
                      dim[1] / len(players) - len(players) * row_gap - 2 * padding[1]))

        for i, player in enumerate(players):
            player_rect = pygame.Rect(pos[0] + padding[0], pos[1] + padding[1] + i * (
                player_dim[1] + row_gap), player_dim[0], player_dim[1])
            player_rect.inflate(-2 * bdrad, -2 * bdrad)
            pygame.draw.rect(
                self.game.screen, player_bg if player['active'] else player_inactive_bg, player_rect, border_radius=bdrad)
            ...
            while True:
                text = self.game.text_surface(player_name,
                                              'Segoe UI Symbol', player_dim[1] / 2, text_color)
            ...
            self.game.draw(
                text, (player_rect.x + padding[0], player_rect.y + player_rect.height // 2 - text.get_height() // 2))
            ...
            status_text = self.game.text_surface(status,
                                                 'Segoe UI Symbol', player_dim[1] / 2, status_color)
            self.game.draw(status_text, (player_rect.x + player_rect.width -
                           padding[0] * 1.5, player_rect.y + player_rect.height // 2 - status_text.get_height() // 2))
